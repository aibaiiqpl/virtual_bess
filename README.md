...
Virtual BESS 起源于 EMS 开发过程中的联调和回归测试需求。真实储能项目里，EMS 经常需要同时对接 PCS、BMS、电表、光伏逆变器、负载和 IEC 61850 设备；但在日常开发、CI、问题复现和方案验证时，开发者通常拿不到完整的现场物理设备。

这个项目用软件模拟一个可配置的虚拟储能场站，让 EMS 可以在没有真实设备的情况下验证南向采集、控制下发、点表映射、聚合计算、异常状态和协议接入逻辑。它不是为了替代现场验收，而是把大量原本依赖现场窗口的测试前移到本地和自动化环境中。
...
- 为 BMS 增加 CAN 协议模拟，覆盖更多真实电池系统的接入方式。
- 扩展 BMS 故障场景，例如单体过压/欠压、温度异常、绝缘异常、通信异常等，用于验证 EMS 告警、保护和历史归档链路。
