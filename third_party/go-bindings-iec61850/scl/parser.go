					...
					if lNodeType, ok := logicNode.SclType.(*LogicalNodeType); ok {
						...
						for _, scb := range logicNode.GSEControlBlocks {
							if scb.Type != "" && scb.Type != "GOOSE" {
								return nil, fmt.Errorf("GSEControl of type %s not supported!", scb.Type) //nolint:staticcheck
							}
						}
						...
						if !(logicNode.LnClass == "LLN0") && len(logicNode.SettingGroupControlBlocks) > 0 { //nolint:staticcheck // De Morgan's law is not clearer
						...
						for _, doiNode := range logicNode.DOINodes {
							...
							if err = p.parseSubDataInstances(doiNode, dataModelNode); err != nil {
								return nil, err
							}
						}
