...
func (s *StaticModelGenerator) printSettingControlBlock(lnPrefix string, logicalNode *LogicalNode) {
	...
	if len(settingControls) > 0 {
		...
		_, _ = fmt.Fprintf(&sb, "&%s, ", lnPrefix)
		_, _ = fmt.Fprintf(&sb, "%d, %d, 0, false, 0, 0, ", sgcb.ActSG, sgcb.NumOfSGs)

		if s.currentSGCBVariableNumber < len(s.sgcbVariableNames)-1 {
			_, _ = fmt.Fprintf(&sb, "&%s", s.sgcbVariableNames[s.currentSGCBVariableNumber+1])
		} else {
			_, _ = sb.WriteString("NULL")
		}
...
func (s *StaticModelGenerator) printSVControlBlocks(ldName, lnPrefix string, logicalNode *LogicalNode) {
	...
	for _, svCB := range svControlBlocks {
		...
		if svAddress != nil {
			...
			_, _ = fmt.Fprintf(&svStringBuilder, "\nstatic PhyComAddress %s = {\n", phyComAddrName)
			_, _ = fmt.Fprintf(&svStringBuilder, "  %d,\n", svAddress.VlanPriority)
			_, _ = fmt.Fprintf(&svStringBuilder, "  %d,\n", svAddress.VlanId)
			_, _ = fmt.Fprintf(&svStringBuilder, "  %d,\n", svAddress.AppId)
			svStringBuilder.WriteString("  {")
			for i, mac := range svAddress.MacAddress {
				_, _ = fmt.Fprintf(&svStringBuilder, "0x%s", strconv.FormatInt(int64(mac), 16))
		...
		_, _ = fmt.Fprintf(&svStringBuilder, "SVControlBlock %s = {", smvVariableName)
		_, _ = fmt.Fprintf(&svStringBuilder, "&%s, ", lnPrefix)
		_, _ = fmt.Fprintf(&svStringBuilder, "\"%s\", ", svCB.Name)

		if svCB.SmvID == "" {
			svStringBuilder.WriteString("NULL, ")
		} else {
			_, _ = fmt.Fprintf(&svStringBuilder, "\"%s\", ", svCB.SmvID)
		}

		if svCB.DatSet != "" {
			_, _ = fmt.Fprintf(&svStringBuilder, "\"%s\", ", svCB.DatSet)
		} else {
			svStringBuilder.WriteString("NULL, ")
		}

		_, _ = fmt.Fprintf(&svStringBuilder, "%d, ", svCB.SmvOpts.GetIntValue())
		_, _ = fmt.Fprintf(&svStringBuilder, "%d, ", svCB.SmpMod)
		_, _ = fmt.Fprintf(&svStringBuilder, "%d, ", svCB.SmpRate)
		_, _ = fmt.Fprintf(&svStringBuilder, "%d, ", svCB.ConfRev)

		if svAddress != nil {
			_, _ = fmt.Fprintf(&svStringBuilder, "&%s, ", phyComAddrName)
		} else {
			svStringBuilder.WriteString("NULL, ")
		}
		...
		_, _ = fmt.Fprintf(&svStringBuilder, "%d, ", svCB.NofASDU)
		s.currentSvCBVariableNumber++

		if s.currentSvCBVariableNumber < len(s.smvVariableNames) {
			_, _ = fmt.Fprintf(&svStringBuilder, "&%s", s.smvVariableNames[s.currentSvCBVariableNumber])
		} else {
			svStringBuilder.WriteString("NULL")
		}
	...
	for _, gseControlBlock := range gseControlBlocks {
		...
		if gse != nil {
			...
			if gse.Address != nil {
				...
				_, _ = fmt.Fprintf(&gseStringBuilder, "\nstatic PhyComAddress %s = {\n", phyComAddrName)
				_, _ = fmt.Fprintf(&gseStringBuilder, "  %d,\n", gse.Address.VlanPriority)
				_, _ = fmt.Fprintf(&gseStringBuilder, "  %d,\n", gse.Address.VlanId)
				_, _ = fmt.Fprintf(&gseStringBuilder, "  %d,\n", gse.Address.AppId)
				gseStringBuilder.WriteString("  {")
				for i, mac := range gse.Address.MacAddress {
					_, _ = fmt.Fprintf(&gseStringBuilder, "0x%s", strconv.FormatInt(int64(mac), 16))
			...
			_, _ = fmt.Fprintf(&gseStringBuilder, "GSEControlBlock %s = {", gseVariableName)
			_, _ = fmt.Fprintf(&gseStringBuilder, "&%s, ", lnPrefix)
			_, _ = fmt.Fprintf(&gseStringBuilder, "\"%s\", ", gseControlBlock.Name)

			if gseControlBlock.AppID == "" {
				gseStringBuilder.WriteString("NULL, ")
			} else {
				_, _ = fmt.Fprintf(&gseStringBuilder, "\"%s\", ", gseControlBlock.AppID)
			}

			if gseControlBlock.DatSet != "" {
				_, _ = fmt.Fprintf(&gseStringBuilder, "\"%s\", ", gseControlBlock.DatSet)
			} else {
				gseStringBuilder.WriteString("NULL, ")
			}

			_, _ = fmt.Fprintf(&gseStringBuilder, "%d, ", gseControlBlock.ConfRev)
			_, _ = fmt.Fprintf(&gseStringBuilder, "%t, ", gseControlBlock.FixedOffs)

			if gse.Address != nil {
				_, _ = fmt.Fprintf(&gseStringBuilder, "&%s, ", phyComAddrName)
			} else {
				gseStringBuilder.WriteString("NULL, ")
			}

			_, _ = fmt.Fprintf(&gseStringBuilder, "%d, ", gse.MinTime)
			_, _ = fmt.Fprintf(&gseStringBuilder, "%d, ", gse.MaxTime)

			s.currentGseVariableNumber++

			if s.currentGseVariableNumber < len(s.gseVariableNames) {
				_, _ = fmt.Fprintf(&gseStringBuilder, "&%s", s.gseVariableNames[s.currentGseVariableNumber])
			} else {
				gseStringBuilder.WriteString("NULL")
			}
...
func (s *StaticModelGenerator) printLog(lnPrefix string, log *Log, logNumber int) {
	...
	_, _ = fmt.Fprintf(&logString, "Log %s = {", logVariableName)

	// Add Logical Node reference
	_, _ = fmt.Fprintf(&logString, "&%s, ", lnPrefix)

	// Add Log name
	_, _ = fmt.Fprintf(&logString, "\"%s\", ", log.Name)

	// Add reference to the next log
	s.currentLogVariableNumber++
	if s.currentLogVariableNumber < len(s.logVariableNames) {
		_, _ = fmt.Fprintf(&logString, "&%s", s.logVariableNames[s.currentLogVariableNumber])
	} else {
...
func (s *StaticModelGenerator) printLogControlBlock(logicalDevice *LogicalDevice, lnPrefix string, lcb *LogControl, lcbNumber int) {
	...
	_, _ = fmt.Fprintf(&lcbString, "LogControlBlock %s = {", lcbVariableName)

	// Add Logical Node reference
	_, _ = fmt.Fprintf(&lcbString, "&%s, ", lnPrefix)

	// Add LogControl name
	_, _ = fmt.Fprintf(&lcbString, "\"%s\", ", lcb.Name)

	// Add DataSet
	if lcb.DatSet == "" {
		lcbString.WriteString("NULL, ")
	} else {
		_, _ = fmt.Fprintf(&lcbString, "\"%s\", ", lcb.DatSet)
	}
	...
	if lcb.LogName != "" {
		_, _ = fmt.Fprintf(&lcbString, "\"%s%s\", ", logRef, lcb.LogName)
	} else {
		lcbString.WriteString("NULL, ")
	}
	...
	_, _ = fmt.Fprintf(&lcbString, "%d, ", triggerOps)

	// Add Integration Period
	if lcb.IntgPd != 0 {
		_, _ = fmt.Fprintf(&lcbString, "%d, ", lcb.IntgPd)
	} else {
	...
	if s.currentLcbVariableNumber < len(s.lcbVariableNames) {
		_, _ = fmt.Fprintf(&lcbString, "&%s", s.lcbVariableNames[s.currentLcbVariableNumber])
	} else {
		lcbString.WriteString("NULL")
	}
...
func (s *StaticModelGenerator) printReportControlBlockInstance(lnPrefix string, rcb *ReportControl, index string, reportNumber int, _ int, clientIpAddr []byte) {
	rcbVariableName := fmt.Sprintf("%s_report%d", lnPrefix, reportNumber)

	var rcbString strings.Builder
	_, _ = fmt.Fprintf(&rcbString, "ReportControlBlock %s = {", rcbVariableName)

	// Add Logical Node reference
	_, _ = fmt.Fprintf(&rcbString, "&%s, ", lnPrefix)

	// Add Report Name
	_, _ = fmt.Fprintf(&rcbString, "\"%s%s\", ", rcb.Name, index)

	// Add RptID
	if rcb.RptID == "" {
		rcbString.WriteString("NULL, ")
	} else {
		_, _ = fmt.Fprintf(&rcbString, "\"%s\", ", rcb.RptID)
	}

	// Add Buffered
	_, _ = fmt.Fprintf(&rcbString, "%t, ", rcb.Buffered)

	// Add DataSet
	if rcb.DatSet == "" {
		rcbString.WriteString("NULL, ")
	} else {
		_, _ = fmt.Fprintf(&rcbString, "\"%s\", ", rcb.DatSet)
	}
	...
	if rcb.ConfRev == "" {
		...
		_, _ = fmt.Fprintf(&rcbString, "%s, ", rcb.ConfRev)
	}
	...
	_, _ = fmt.Fprintf(&rcbString, "%d, ", triggerOps)
	...
	_, _ = fmt.Fprintf(&rcbString, "%d, ", options)

	// Add BufferTime
	_, _ = fmt.Fprintf(&rcbString, "%d, ", rcb.BufTime)

	// Add IntegrityPeriod
	if rcb.IntgPd == "" {
		rcbString.WriteString("0, ")
	} else {
		_, _ = fmt.Fprintf(&rcbString, "%s, ", rcb.IntgPd)
	}

	// Add Client IP Address
	rcbString.WriteString("{")
	for i, byteVal := range clientIpAddr {
		_, _ = fmt.Fprintf(&rcbString, "0x%02X", byteVal)
	...
	if s.currentRcbVariableNumber < len(s.rcbVariableNames) {
		_, _ = fmt.Fprintf(&rcbString, "&%s", s.rcbVariableNames[s.currentRcbVariableNumber])
	} else {
		rcbString.WriteString("NULL")
	}
...
func (s *StaticModelGenerator) printDataAttributeDefinitions(doName string, dataAttributes []*DataAttribute, isTransient bool, _ int) {
	for i, dataAttribute := range dataAttributes {
		...
		// If an array, print each array element definition
		if isArray {
...
func (s *StaticModelGenerator) printValue(daName string, dataAttribute *DataAttribute, value *DataModelValue) {
	...
	if s.initializeOnce {
		_, _ = fmt.Fprintf(s.initializerBuffer, "if (%s.mmsValue == nil) {\n", daName)
	}

	_, _ = fmt.Fprintf(s.initializerBuffer, "%s.mmsValue = ", daName)

	// Handle different data types
	switch dataAttribute.AttributeType {
	case Enumerated, Int8, Int16, Int32, Int64:
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newIntegerFromInt32(%d);", cast.ToInt(value.Value))
	case Int8U, Int16U, Int24U, Int32U:
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newUnsignedFromUint32(%d);", cast.ToInt64(value.Value))
	case Boolean:
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newBoolean(%v);", cast.ToBool(value.Value))
	case OctetString64:
		daValName := daName + "__val"
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newOctetString(0, 64);\n")
		_, _ = fmt.Fprintf(s.initializerBuffer, "uint8_t %s[] = ", daValName)
		appendHexArrayString(s.initializerBuffer, value.Value.([]byte))
		_, _ = fmt.Fprintf(s.initializerBuffer, ";\nMmsValue_setOctetString(%s.mmsValue, %s, %d);\n", daName, daValName, len(value.Value.([]byte)))
	case CodedEnum:
		s.initializerBuffer.WriteString("MmsValue_newBitString(2);\n")
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_setBitStringFromIntegerBigEndian(%s.mmsValue, %v);\n", daName, value.Value)
	case UnicodeString255:
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newMmsString(\"%s\");", value.Value)
	case VisibleString32, VisibleString64, VisibleString129, VisibleString255, VisibleString65, Currency:
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newVisibleString(\"%s\");", value.Value)
	case Float32:
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newFloat(%v);", value.Value)
	case Float64:
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newDouble(%v);", value.Value)
	case Timestamp:
		_, _ = fmt.Fprintf(s.initializerBuffer, "MmsValue_newUtcTimeByMsTime(%v);", value.Value)
	default:
...
func appendHexArrayString(builder *strings.Builder, byteArray []byte) {
	...
	for i, b := range byteArray {
		if i == 0 {
			_, _ = fmt.Fprintf(builder, "0x%02X", b)
		} else {
			_, _ = fmt.Fprintf(builder, ", 0x%02X", b)
		}
	}
...
func toMmsString(iecString string) string {
	return strings.ReplaceAll(iecString, ".", "$")
}
